...
### 命令层次结构
我们选择了子命令 schema（`spec show`、`spec list`、`spec validate`）以：
- 在公共命名空间下对相关功能进行分组
- 实现未来可扩展性而不污染顶级 CLI
- 保持与计划中的 `change` 命令结构的一致性

### JSON schema 结构
Spec JSON schema 遵循以下结构：
```typescript
{
 version: string, // 用于兼容性的 schema 版本
 format: "spec", // 标识为 spec 文档
 sourcePath: string, // 原始 Markdown 文件路径
 id: string, // 来自文件名的 spec 标识符
...
### 验证策略
- **解析时验证**：在解析期间捕获结构性问题
- **schema 验证**：使用 Zod 对解析后的数据进行运行时类型检查
