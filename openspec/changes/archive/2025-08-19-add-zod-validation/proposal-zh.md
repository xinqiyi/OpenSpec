...
虽然 spec 和 change 命令可以输出 JSON，但它们目前不执行超出基本结构检查的严格运行时验证。这可能导致无效的 spec 或变更被处理、必需字段缺失时的静默失败以及糟糕的错误消息。
...
- 提供 JSON 格式的详细验证报告
- 添加在警告上失败的 `--strict` schema

## 影响

- **受影响的 spec**：cli-spec、cli-change、cli-archive、cli-diff
- **受影响的代码**：
