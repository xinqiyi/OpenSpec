...
- [x] 2.4 当未提供 `[spec-id]` 且允许交互时，提示从 `openspec/specs/*/spec.md` 中选择并验证所选规范
...
- [x] 6.2 在 `--json` 模式下保持输出稳定（无额外日志到 stdout；如有需要将进度输出到 stderr）
- [x] 6.3 通过并发限制确保响应性
...
- [x] 8.3 确保 `openspec/changes/bulk-validation-interactive-selection/specs/*` 下现有规范仍然满足
...
## 10. 验收标准映射
- [x] AC-1：`openspec change validate` 无参数时交互式选择（仅 TTY；遵循 `--no-interactive`/环境变量）—— 符合 cli-change 规范
- [x] AC-2：`openspec spec validate` 无参数时交互式选择（仅 TTY；遵循 `--no-interactive`/环境变量）—— 符合 cli-spec 规范
- [x] AC-3：新的 `openspec validate` 支持交互式选择、批量/过滤验证、JSON 模式、进度、并发、退出码 —— 符合 cli-validate 规范
