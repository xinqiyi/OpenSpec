...
- 保持现有的 `openspec/AGENTS.md` 模板作为规范来源，同时确保读取 `AGENTS.md` 选择加入指令的助手自动获取最新指导。
...
2. 生成文件时，从与 `openspec/AGENTS.md` 相同的模板中提取受管内容，确保两个位置保持同步。
3. 更新 `openspec update`，使其始终刷新根 `AGENTS.md`（如果缺失则创建），与 `openspec/AGENTS.md` 和其他已配置的助手一起。
4. 在 CLI 规范中记录新行为，并用两个命令的测试验证标记处理（无重复，保留块外用户内容）。
