...
一些单元测试（例如 ChangeCommand.show/validate）通过 `process.cwd()` 和 `openspec/changes` 读取实时仓库状态。这使得结果依赖于碰巧存在的任何目录以及 `fs.readdir` 返回的顺序，导致不同环境下的成功/失败不稳定。
...
- 小而专注的更改消除了不稳定性，而不改变用户工作流。
- 临时夹具是一种广为人知的测试模式，保持测试快速。
...
- 阶段 2：将相同模式应用于任何剩下的触及文件发现的套件（`list`、`show`、`validate`、`diff`）。
