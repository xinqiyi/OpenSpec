...
3. **价值有限**：开发者可以使用现有工具实现更好的差异可视化：
 ...
 - 使用 `show` 命令进行结构化变更查看
 - 使用标准差异工具直接比较 spec 文件
4. **与动词-名词 schema 不一致**：该命令不遵循其他命令正在迁移的首选动词优先命令结构
...
- **更清晰的用户路径**：引导用户使用 spec 的 `show` 命令查看变更
- **更低的维护成本**：需要维护和测试的命令更少
- **更好的一致性**：专注于核心 OpenSpec workflow，无需冗余功能

## 实施

### 要删除的文件
- `/src/core/diff.ts` - 整个 diff 命令实现
- `/openspec/specs/cli-diff/spec.md` - diff 命令 spec
...
- **用户干扰**：现有用户的 workflow 可能依赖于 diff 命令
 - 缓解措施：提供清晰的迁移指南和弃用期
