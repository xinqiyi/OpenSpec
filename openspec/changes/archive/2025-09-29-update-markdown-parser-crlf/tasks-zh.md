...
## 2. spec 化解析
- [x] 2.1 在构造 `MarkdownParser` 时 spec 化换行符，使标题和内容比较忽略 `\r`。
- [x] 2.2 确保所有 CLI 入口点（validate、view、spec 转换）都复用 spec 化的解析器路径。

## 3. 文档与验证
- [x] 3.1 更新 `cli-validate` spec，添加一个覆盖 CRLF 换行符的场景。
