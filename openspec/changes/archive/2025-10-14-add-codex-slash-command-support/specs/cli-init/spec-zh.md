...
- **并且** 在已配置工具旁边显示"（已配置）"，使用户了解选择将刷新内容
- **并且** 将禁用的工具视为"即将推出"并保持不可选状态
- **并且** 允许在选择一个或多个工具后按 Enter 确认
...
- **那么** 创建 `.opencode/command/openspec-proposal.md`、`.opencode/command/openspec-apply.md` 和 `.opencode/command/openspec-archive.md`
- **并且** 从共享模板填充每个文件，使命令文本与其他工具一致
- **并且** 每个模板包含相关 OpenSpec 工作流阶段的说明
