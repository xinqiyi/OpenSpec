...
- [x] 1.2 注册一个 `WindsurfSlashCommandConfigurator`，将工作流写入 `.windsurf/workflows/`，并确保目录存在。
...
- [x] 3.2 扩展 update 测试，断言现有的 Windsurf 工作流被刷新，不存在的文件被忽略。
- [x] 3.3 添加 Windsurf 工作流文件中标记保留的回归覆盖。

## 4. 文档
- [x] 4.1 更新 README（及任何面向用户的文档），在原生斜杠/工作流集成下列出 Windsurf。
- [x] 4.2 在发布说明或 CHANGELOG 中提及 Windsurf 工作流支持（如适用）。
