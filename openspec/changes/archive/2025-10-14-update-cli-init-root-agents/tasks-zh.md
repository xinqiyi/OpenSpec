## 1. 实现
- [x] 1.1 重构 `openspec init`，使其始终通过共享辅助逻辑生成根级 `AGENTS.md` 存根（首次运行和扩展模式）。
- [x] 1.2 重新设计 AI 工具选择向导，展示"原生支持"与"其他工具"分组，并使存根成为非可选项。
- [x] 1.3 更新 CLI 消息、模板和配置器，使新流程在 init 和 update 命令之间保持同步。
...
- [x] 2.3 执行手动冒烟测试：在临时目录中运行 `openspec init`，确认存根和分组提示，然后在扩展模式下再次运行。
