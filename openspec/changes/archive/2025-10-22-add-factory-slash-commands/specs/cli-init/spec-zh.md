...
- **WHEN** 用户在初始化过程中选择 Factory Droid
- **THEN** 创建 `.factory/commands/openspec-proposal.md`、`.factory/commands/openspec-apply.md` 和 `.factory/commands/openspec-archive.md`
- **AND** 使用包含 `description` 和 `argument-hint` 字段的 Factory 兼容 YAML frontmatter 的共享 template 填充每个文件
- **AND** 在 template 主体中包含 `$ARGUMENTS` 占位符，以便 droid 接收用户提供的任何输入
- **AND** 将生成的内容包裹在 OpenSpec 管理标记中，以便 `openspec update` 可以安全地刷新命令
