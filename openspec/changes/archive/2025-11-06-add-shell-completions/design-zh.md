...
本设计建立了一个基于插件的 shell 补全架构，优先考虑清晰的 TypeScript schema、可扩展性和可维护性。该系统分离了 shell 特定的生成逻辑、动态补全数据提供者和安装自动化之间的关注点。

**范围：** 本 proposal 实现了 **仅 Zsh 补全**（Oh My Zsh 优先）。该架构设计用于在未来的 proposal 中支持 bash、fish 和 PowerShell。

## 原生 Shell 补全行为

**设计理念：** 我们与每个 shell 的原生补全系统集成，而不是尝试自定义或统一其行为。这确保了用户熟悉的体验并降低了维护复杂度。

**注意：** 虽然以下记录了所有四种 shell 的行为作为架构参考，**本 proposal 仅实现了 Zsh**。Bash、Fish 和 PowerShell 的文档旨在指导未来的实现。
...
每个 shell 都有独特的补全语法和约定。我们不创建带有分支逻辑的单一生成器，而是使用插件 schema，每个 shell 实现一个公共接口：
...
### 决策：独立生成器与 template 引擎

**选择：** 每个 shell 使用独立的生成器类

**替代方案：** 使用 template 引擎配合 shell 特定 template

**理由：**
- Shell 补全语法本质上是不同的（不仅仅是文本替换）
- 类比 template 提供更好的类型安全
- 逻辑复杂度（缓存、动态补全）不适合 template 范式
- 专门的类更易于调试和测试
...
- 减少手动配置造成的错误
- 符合用户对现代 CLI 工具的期望
- 仍支持通过脚本生成到 stdout 的手动 workflow 程
