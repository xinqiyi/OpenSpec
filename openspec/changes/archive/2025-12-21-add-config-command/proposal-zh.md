...
用户需要一种方式来查看和修改他们的全局 OpenSpec 设置，而无需手动编辑 JSON 文件。`global-config` spec 提供了基础，但没有面向用户的界面来与配置交互。专门的 `openspec config` 命令提供了可发现性和易用性。
