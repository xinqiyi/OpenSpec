## 1. 类型定义
- [x] 1.1 创建 `src/core/artifact-graph/types.ts`，包含 Zod 模式（`ArtifactSchema`、`SchemaYamlSchema`）和通过 `z.infer<>` 推导的类型
...
- [x] 2.3 实现重复制品 ID 检测
- [x] 2.4 在模式加载期间添加循环检测（错误格式："检测到循环依赖：A → B → C → A"）

## 3. 制品图核心
- [x] 3.1 创建包含 ArtifactGraph 类的 `src/core/artifact-graph/graph.ts`
- [x] 3.2 实现 `fromYaml(path)` - 从模式文件加载图
...
## 6. 模式解析
- [x] 6.1 创建包含模式解析逻辑的 `src/core/artifact-graph/resolver.ts`
...
## 9. 测试
- [x] 9.1 测试：解析有效模式 YAML 返回正确的制品图
- [x] 9.2 测试：解析无效模式（缺少字段）抛出描述性错误
- [x] 9.3 测试：重复制品 ID 抛出错误
- [x] 9.4 测试：无效的 `requires` 引用抛出错误，标识无效的 ID
- [x] 9.5 测试：模式中的循环抛出错误，列出循环路径（例如 "A → B → C → A"）
...
- [x] 9.11 测试：Glob 模式 specs/*.md 在文件存在时检测为完成
- [x] 9.12 测试：Glob 模式空目录不标记为完成
...
- [x] 9.18 测试：模式解析在全局覆盖之前优先于内置
- [x] 9.19 测试：模式解析在无全局时回退到内置
