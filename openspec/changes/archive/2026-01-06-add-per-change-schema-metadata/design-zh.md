...
实验性 artifact workflow 支持多种架构（`spec-driven`、`tdd`），但架构选择必须在每个命令上传递。这给 agent 和用户带来了摩擦。
...
- 遵循现有代码库 schema（参见 `ArtifactSchema`、`SchemaYamlSchema`）
