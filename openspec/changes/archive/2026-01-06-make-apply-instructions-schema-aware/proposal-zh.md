...
`generateApplyInstructions` 函数硬编码检查 `spec-driven` 的工件（`proposal.md`、`specs/`、`design.md`、`tasks.md`）。如果用户选择不同的 schema（如 `tdd`），apply 指令将毫无意义——它检查的是该 schema 中不存在的文件。

这阻碍了实验性工作流正确支持多个 schema。

## 变更内容

**范围：实验性工件工作流**（`openspec instructions apply`）

**依赖：** `add-per-change-schema-metadata`（用于了解变更使用哪个 schema）

- 使 `generateApplyInstructions` 从 schema 读取工件定义
- 根据 schema 动态确定哪些工件存在
...
artifacts:
  - id: tasks
    generates: tasks.md
    apply: true  # ← 此工件触发 apply 模式
```

**优点：** 显式、灵活
**缺点：** 需要维护的额外字段，如果有多个工件是 `apply: true` 怎么办？

### 方案 B：叶子工件是可实现的

没有依赖项（无其他工件依赖它们）的工件是 apply 目标。
...
### 方案 D：基于约定的匹配（工件 ID 匹配）

如果工件 ID 是 `tasks` 或 `implementation`，则它是 apply 目标。

**优点：** 简单，无需 schema 变更
**缺点：** 脆弱，不适用于自定义 schema

### 方案 E：所有工件完成 → apply 可用

当所有 schema 工件事务存在时，apply 可用。实现是用户在规划后所做的任何事情。

**优点：** 简单，无需 schema 变更
**缺点：** 无法指导"apply"对不同工作流的含义
...
name: spec-driven
version: 1
description: 默认 OpenSpec 工作流

artifacts:
  # ... 现有工件 ...

apply:
  requires: [tasks]           # apply 前必须存在的工件
  tracks: tasks.md            # 带有复选框进度的文件（可选）
  instruction: |              # 显示给代理的指导
...
**关键属性：**
- `requires`：在 apply 可用之前必须存在的工件 ID 数组
- `tracks`：带有复选框的文件路径（相对于变更目录），如果无跟踪则为 `null`
- `instruction`：apply 阶段的自定义指导

**回退行为：** 没有 `apply` 块的 schema 默认使用"所有工件必须存在"
