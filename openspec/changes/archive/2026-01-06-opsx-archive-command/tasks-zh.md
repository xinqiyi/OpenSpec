...
- [x] 1.1 创建包含 skill 定义的 `.claude/commands/opsx/archive.md`
...
- [x] 1.6 实现 spec 同步提示（检查 specs/ 目录，提供 `/opsx:sync`）
- [x] 1.7 实现 archive 过程（移动到 archive/YYYY-MM-DD-<name>/）
- [x] 1.8 为成功/警告情况添加输出格式化

## 2. 重新生成 skill

- [x] 2.1 运行 `openspec artifact-experimental-setup` 以重新生成 skill
- [x] 2.2 验证 skill 出现在 `.claude/skills/` 目录中
...
- [x] 3.4 测试 `/opsx:archive` 与 delta spec（验证显示同步提示）
