...
目前，没有 CI 验证来确保这些 Nix artifact 保持可用。现有的 CI workflow（.github/workflows/ci.yml）验证多个平台（Linux、macOS、Windows）上的 Node.js 构建、测试和代码检查，但不验证 Nix 构建。
...
- 单独的 workflow 文件：对于单个任务来说过于复杂，更难管理必需检查
...
- 测试用户在发布时将遵循的完整 workflow
- 及早发现错误

**实施方法**：
- 以不修改 git 状态的方式执行脚本（或之后丢弃更改）
- 验证脚本以代码 0 退出
- 可选地，验证执行后 flake.nix 包含预期的 schema

**考虑的替代方案**：
- 模拟/dry-run schema：需要大幅修改脚本
...
**内容**：确保 workflow 与 `act` 工具兼容，用于本地 CI 测试。
...
**影响**：由于 Docker 环境差异，workflow 可能无法与 act 完美配合。
