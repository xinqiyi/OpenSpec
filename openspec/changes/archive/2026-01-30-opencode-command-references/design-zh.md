...
- 通过 `src/core/shared/skill-generation.ts` 中的 `generateSkillContent()` 生成的**技能**

目前：
- 命令通过适配器系统处理，可以按工具转换内容
- 技能使用单一的共享函数，没有针对特定工具的转换

`src/core/templates/skill-templates.ts` 中的模板使用 Claude 的冒号格式（`/opsx:new`）作为规范格式。使用不同格式的工具需要在生成时进行转换。

## 目标 / 非目标

**目标：**
- 为 OpenCode 在命令和技能中将所有 `/opsx:` 命令引用转换为 `/opsx-`
- 创建一个共享的、可复用的转换工具
- 通过回调参数保持转换的可选性（非硬编码工具检测）

**非目标：**
- 修改规范模板格式（模板保持使用 `/opsx:`）
- 对其他工具应用转换（目前仅限 OpenCode）
- 为技能创建完整的适配器系统（当前需求无需过度设计）
...
| 风险 | 缓解措施 |
|------|----------|
| 存在不应转换的其他 `/opsx:` 模式 | 模板中的所有出现都是命令调用 - 通过检查验证 |
