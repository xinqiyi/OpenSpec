...
OpenCode 使用连字符命令语法（`/opsx-new`），但我们的模板包含冒号引用（`/opsx:new`）。这导致生成的命令文件和技能文件包含与实际命令调用语法不匹配的引用，使 AI 和用户都感到困惑。
...
- 更新 `init.ts` 和 `update.ts`，在为 OpenCode 生成技能时传递转换器
...
无 - 无规范级别的行为变更。这是 OpenCode 适配器和技能生成中的实现修复，不改变任何外部需求或合同。
...
- **用户**：OpenCode 用户将在生成的命令文件和技能文件中看到正确的 `/opsx-` 命令引用
