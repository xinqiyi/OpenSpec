# 变更：添加 /opsx:verify skill

## 原因

用户需要一种方法来验证他们的实现与实际请求是否匹配，然后才能 archive 变更。目前，没有系统化的方式来检查：
- 所有任务是否真正完成
- 实现是否覆盖了所有 spec 需求和场景
- 实现是否遵循了设计决策
- 代码是否连贯且合理
...
- 将 verify skill 集成到 `artifact-workflow.ts` 的 `artifactExperimentalSetupCommand` 中
- 在设置命令的 skills 和 commands 数组中添加 verify
- 更新帮助文本，在可用命令列表中包含 `/opsx:verify`
- 创建 `opsx-verify-skill` 能力 spec

## 验证维度

该 skill 从三个维度进行验证：

1. **完整性** - 所有任务是否完成？所有 spec 是否已处理？
2. **正确性** - 实现是否与 spec 匹配？场景是否已覆盖？
...
- 受影响的 spec：新的 `opsx-verify-skill` spec
- 受影响的代码：
 - `src/core/templates/skill-templates.ts` - 新增 2 个 template 函数
...
- 相关 skill：与 `/opsx:apply` 配合使用，在 `/opsx:archive` 之前
