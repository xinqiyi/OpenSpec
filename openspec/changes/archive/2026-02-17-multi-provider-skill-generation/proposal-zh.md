...
`artifact-experimental-setup` 命令目前将 skill 输出路径硬编码为 `.claude/skills` 和 `.claude/commands/opsx`。这阻止了其他 AI 编码工具（Cursor、Windsurf、Codex 等）的用户使用 OpenSpec 的 skill 生成功能。我们需要支持多样化的 AI 编码助手生态系统，每个助手都有自己的 skill/指令文件位置和命令 frontmatter 格式约定。

## 变更内容

- 在 `config.ts` 的现有 `AIToolOption` 接口中添加 `skillsDir` 路径配置
- 向 `artifact-experimental-setup` 命令添加必需的 `--tool <tool-id>` 标志
- 使用策略/适配器 schema 创建通用的命令生成系统：
...
- `ai-tool-paths`：将 AI 工具 ID 映射到其项目本地 skill 目录路径的配置
...
- **向后兼容性**：现有 workflow 不受影响——这是一个新的命令设置功能
