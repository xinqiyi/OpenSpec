...
- **THEN** skill 文件生成在 `.cursor/skills/` 中
- **AND** 命令文件使用 Cursor 的 frontmatter 格式生成

#### 场景：缺少工具标志

- **WHEN** 用户运行 `openspec artifact-experimental-setup` 时未带 `--tool`
...
- **THEN** 系统显示错误，列出有效的工具 ID

#### 场景：没有 skillsDir 的工具

- **WHEN** 用户指定的工具没有配置 `skillsDir`
- **THEN** 系统显示错误，指示该工具不支持 skill 生成

#### 场景：没有命令适配器的工具

- **WHEN** 用户指定的工具有 `skillsDir` 但没有注册命令适配器
- **THEN** skill 文件生成成功
- **AND** 命令生成被跳过并显示信息性消息
...
- **WHEN** 设置命令成功运行
- **THEN** 输出包含目标工具名称（例如"正在为 Cursor 设置..."）
...
- **WHEN** 由于缺少适配器而跳过命令生成
- **THEN** 输出包含消息："命令生成已跳过 - 没有适用于 <tool> 的适配器"
