...
定义一个通用命令生成系统，通过策略/适配器模式支持多种 AI 工具，将命令内容与工具特定格式分离。
...
- **则** 适配器应输出包含 `name`、`description`、`category`、`tags` 字段的 YAML frontmatter
- **并且** 文件路径应遵循模式 `.claude/commands/opsx/<id>.md`

#### 场景：Cursor 适配器格式化

- **当** 为 Cursor 格式化命令时
- **则** 适配器应输出包含 `name`（格式为 `/opsx-<id>`）、`id`、`category`、`description` 字段的 YAML frontmatter
...
- **当** 调用 `generateCommand(content, adapter)`
- **则** 应返回一个包含以下内容的对象：
...
- **当** 调用 `CommandAdapterRegistry.get('cursor')`
- **则** 应返回 Cursor 适配器，如果未注册则返回 undefined

#### 场景：获取所有适配器

- **当** 调用 `CommandAdapterRegistry.getAll()`
- **则** 应返回所有已注册适配器的数组

#### 场景：未找到适配器

- **当** 查找未注册工具的适配器时
- **则** `CommandAdapterRegistry.get()` 应返回 undefined
- **并且** 调用方应适当处理缺少适配器的情况
