...
OpenSpec 目前使用固定的 schema 解析顺序：
1. `--schema` CLI 标志
2. change 目录中的 `.openspec.yaml`
3. 硬编码默认值：`"spec-driven"`

这迫使用户即使只是为了简单的需求（如注入技术栈上下文或添加特定 artifact 的规则），也需要 fork 整个 schema 来获得项目级别的自定义能力。

本 proposal 引入 `openspec/config.yaml` 作为一个轻量级自定义层，位于预设 schema 和完整 fork 之间。它允许团队：
- 设置默认 schema
- 将项目上下文注入所有 artifact
- 添加每个 artifact 的规则
...
**约束条件：**
- 不能破坏缺少配置的现有 change
- 必须在"配置"（本功能）和"fork"（项目本地 schema）之间保持清晰分离
- 配置仅限项目级别（无全局/用户级别配置）

**关键利益相关方：**
- 需要轻量自定义而无需 fork 的 OpenSpec 用户
- 通过提交的配置共享 workflow 约定的团队

## 目标 / 非目标

**目标：**
- 使用 Zod schema 加载和解析 `openspec/config.yaml`
- 在 schema 解析中使用配置的 `schema` 字段作为默认值
- 将 `context` 注入所有 artifact 指令
- 仅将 `rules` 注入匹配的 artifact 指令
- 优雅处理缺失或无效的配置（回退到默认值）

**非目标：**
- 对 schema 进行结构性更改（`skip`、`add`、继承）——这些属于 fork 路径
...
**理由：**
- 与 `openspec/schemas/`（项目本地 schema）放在同一位置
- 保持项目根目录整洁
- OpenSpec 配置的自然命名空间
...
**理由：**
- 面向未来：如果 schema 添加新 artifact，旧配置不会出错
- 开发体验：拼写错误显示警告，但不中断 workflow
- 用户可以 delta 修复
...
rules:
 proposal: [...]
 testplan: [...] # schema 没有这个 artifact → WARN，不是 ERROR
```
 ...
 * 目的：
 * 1. 文档——清晰定义配置文件结构
 * 2. 类型安全——TypeScript 从 schema 推断 ProjectConfig 类型
 * 3. 运行时验证——使用 safeParse() 进行弹性的逐字段验证
 *
 * 为什么选择 Zod 而不是手动验证：
 * - 帮助快速了解 OpenSpec 的数据接口
 * - 类型和验证的单一可信源
 * - 与其他 OpenSpec schema 保持一致
 */
...
/**
 * 根据 schema 的 artifact 验证规则中的 artifact ID。
 * 在指令加载时调用（当 schema 已知时）。
...
/**
 * 当用户提供无效 schema 时建议有效的 schema 名称。
 * 使用模糊匹配查找相似名称。
 */
export function suggestSchemas(
 ...
 message += `\n 修复方式：编辑 openspec/config.yaml，将 'schema: ${invalidSchemaName}' 改为有效的 schema 名称`;
...
更新 `createNewChange()` 以使用配置 schema：
...
export function loadInstructions(
 ...
 const schema = getSchemaForChange(changeName); // 假设已加载 schema
...
**问：如果配置的 schema 字段引用了不存在的 schema 怎么办？**
**答：** schema 解析会在下游失败。在尝试加载 schema 时显示错误，建议有效的 schema 名称。

**问：规则是否应根据已解析 schema 的 artifact ID 进行验证？**
**答：** 是的，验证并警告，但不中断。这允许在 schema 演化时向前兼容。
