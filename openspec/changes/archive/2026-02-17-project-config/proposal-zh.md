...
添加 `openspec/config.yaml` 支持，用于项目级别的配置。这使得团队无需 fork schema 即可自定义 OpenSpec 行为，通过提供注入到 artifact 生成中的上下文和规则来实现。

## 动机

目前，自定义 OpenSpec 需要 fork 整个 schema：
- 即使只添加一条规则也必须复制所有文件
- 当 openspec 升级时会丢失更新
- 对于简单的自定义操作来说门槛过高

大多数用户不需要不同的 workflow 结构。他们需要的是：
- 提供项目上下文（技术栈、约定、约束）
...
┌─────────────────────────────────────────────────────────────────┐
│ │
│ 配置（本次变更） FORK（项目本地 schema） │
│ ───────────────────── ──────────────────────────── │
│ │
│ 使用预设 schema 从头定义自己的 schema │
│ + 添加上下文 │
│ + 添加规则 │
│ │
...
使用哪个 workflow schema。可以是：
- 内置名称：`spec-driven`、`tdd`
- 项目本地 schema 名称：`my-workflow`（需要 project-local-schemas 变更）
...
每个 artifact 的规则，以 artifact ID 为键。对 schema 内置指导的附加补充。
...
此功能面向**实验性 workflow 用户**。在实验性设置期间创建配置（而非提供独立命令）是有意为之的：

**理由：**
1. **单一入口点**——正在设置实验性功能的用户已经处于"配置 schema"
2. **上下文时机**——在设置 workflow 时配置项目默认值很自然
...
**核心配置系统：**
- 使用 Zod schema 定义 `ProjectConfig` 类型
- 添加带有优雅错误处理的 `readProjectConfig()` 函数
- 更新指令生成以注入上下文（所有 artifact）
- 更新指令生成以注入规则（每个 artifact）
- 更新 schema 解析以使用配置的 `schema` 字段作为默认值
- 更新 `openspec new change` 以使用配置的 schema 作为默认值

**配置创建（实验性设置）：**
- 扩展 `artifact-experimental-setup` 命令，可选创建配置
- schema 选择的交互式提示（含每个 schema 的描述）
- 项目上下文的交互式提示（可选多行输入）
...
当用户设置实验性 workflow 时，他们会被提示可选地创建配置：

```bash
$ openspec artifact-experimental-setup

正在设置实验性 artifact workflow...
...
使用方法：
 • 新 change 自动使用 'spec-driven' schema
 • 上下文注入到所有 artifact 指令中
 • 规则应用于匹配的 artifact

与团队共享：
 git add openspec/config.yaml .claude/
 git commit -m "使用项目配置设置 OpenSpec 实验性 workflow"
...
3. **引导式提示**——schema 描述、上下文示例、artifact 选择
4. **即时验证**——创建后立即验证配置，错误即时显示
5. **清晰输出**——精确显示创建了什么及其对 workflow 的影响
...
# 配置后（含 schema: spec-driven）：schema 自动确定
/opsx:new my-feature
...
| `src/utils/change-utils.ts` | 在 `createChange()` 中使用配置 schema 作为默认值 |
...
- `schema` 必须是有效的 schema 名称（在解析中存在）
- `context` 必须是字符串
...
3. **提示：schema 选择**
 - 使用 `listSchemasWithInfo()` 获取可用 schema
 - 显示每个 schema 的描述和 artifact 流程
 - 默认选择第一个 schema（通常是 "spec-driven"）
...
7. **显示成功摘要：**
 - 创建的配置路径
 - 摘要：使用的 schema、添加上下文（行数）、规则数量
 - 展示配置如何影响 workflow 的使用示例
 - 建议将配置提交到 git

**错误处理：**
- 无效的 schema 选择 → 显示带有模糊匹配建议的可用 schema，重试
- 上下文过大（>50KB）→ 拒绝并报错，要求减小大小
- 规则引用无效 artifact → 警告但继续（向前兼容）
- 文件写入失败 → 显示错误，建议手动创建
- 配置已存在 → 显示消息，跳过配置章节，继续设置
- 用户取消（Ctrl+C）→ 记录"配置创建已取消"，继续执行设置的其余部分（skill/命令已创建）
...
- **project-local-schemas**：使 `schema: my-workflow` 可以引用项目本地 schema
...
// Zod schema 同时作为运行时验证和文档
// 类型从 schema 推断以获得类型安全
export const ProjectConfigSchema = z.object({
 // 必填：使用哪个 schema（例如 "spec-driven", "tdd"，或项目本地 schema 名称）
 schema: z.string().min(1).describe('要使用的 workflow schema（例如 "spec-driven", "tdd"）'),

 // 可选：项目上下文（注入到所有 artifact 指令中）
 // 最大大小：50KB（解析时强制）
 context: z.string().optional().describe('注入到所有 artifact 指令的项目上下文'),

 // 可选：每个 artifact 的规则（对 schema 内置指导的补充）
 rules: z.record(
