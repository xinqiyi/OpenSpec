...
- [x] 1.5 实现 `validateConfigRules()`，验证 artifact ID 是否与模式匹配（在指令加载时调用）
...
- [x] 1.10 添加带拼写错误的模糊模式匹配的单元测试

## 2. 模式解析集成

- [x] 2.1 更新 `src/utils/change-metadata.ts` 中的 `resolveSchemaForChange()`，检查项目配置（优先级第三）
- [x] 2.2 更新 `src/utils/change-utils.ts` 中的 `createNewChange()`，使用配置模式作为默认值
- [x] 2.3 为模式解析优先级添加集成测试（CLI → change 元数据 → 配置 → 默认值）
- [x] 2.4 添加配置中项目本地模式名称的测试
- [x] 2.5 添加不存在的模式错误处理及建议的测试
...
- [x] 5.2 在技能/命令创建之后添加配置创建部分，包含标题和描述
...
- [x] 5.6 显示成功摘要，包含路径、模式、上下文行数、规则数量
- [x] 5.7 显示使用示例和 git 提交建议
- [x] 5.8 处理配置已存在的情况，显示跳过消息和手动更新说明
- [x] 5.9 为文件写入失败添加错误处理，包含回退建议
- [x] 5.10 添加取消行为的测试（技能/命令保留，配置未创建）

## 6. 测试和文档

- [x] 6.1 添加端到端测试：运行实验性设置 → 创建配置 → 创建 change → 验证使用的模式
...
- [x] 6.10 使用配置功能示例和模式更新 README 或文档
- [x] 6.11 记录不同模式的常见 artifact ID
