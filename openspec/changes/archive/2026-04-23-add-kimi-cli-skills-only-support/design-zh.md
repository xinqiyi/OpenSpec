...
Kimi CLI 不是另一个 Claude/Codex 风格的适配器目标。它的扩展模型围绕发现的技能构建，而不是外部命令文件：
...
- `init` 可以为任何选中的带有 `skillsDir` 的工具安装技能
- 当尝试为没有适配器的工具生成命令时，OpenSpec 已经记录了 `commandsSkipped`

## 目标

- 使用与 Trae 相同的狭窄 `skills-only` 模式添加 Kimi CLI
- 保持实现小巧：元数据、文档和一个针对性的回归测试
- 使规范文本匹配当前无适配器工具的代码路径
...
这与 Kimi CLI 的项目本地技能根目录匹配，并允许现有的 init/update 检测路径将其视为受支持的工具。
...
主要的权衡是范围：Kimi 将继承当前无适配器工具的行为，包括 `delivery=commands` 对于可技能调用的工具尚未具备能力感知的广泛限制。对于此变更是可接受的，因为它匹配现有的 Trae/ForgeCode 模型，并使实现与已验证的 Kimi CLI 行为保持一致。
