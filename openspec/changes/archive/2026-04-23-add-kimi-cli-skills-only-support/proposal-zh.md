...
OpenSpec 已经支持安装技能而无需命令适配器的工具。Trae 和 ForgeCode 是现有的例子。Kimi 应遵循相同的模式，而不是引入未记录的 `.kimi/commands/...` 行为。

## 变更内容

- 在 `AI_TOOLS` 中添加 Kimi CLI 作为受支持的工具，包含 `skillsDir: '.kimi'`
- 在支持工具和命令使用文档中，将 Kimi CLI 记录为纯技能集成
- 调整变更规范，使 `cli-init` 明确允许选中的带有 `skillsDir` 但无注册命令适配器的工具
...
- `ai-tool-paths`：为 Kimi CLI 定义 `.kimi` 技能根目录
...
更广泛的能力感知交付工作已在 `add-tool-command-surface-capabilities` 中单独探索。此变更保持狭窄，并遵循现有的 Trae/ForgeCode 模式。
