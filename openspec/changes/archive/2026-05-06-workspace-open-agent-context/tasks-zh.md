...
- [x] 1.1 为 workspace 本地状态解析和序列化添加结构化的 `preferred_opener` 支持
- [x] 1.2 在添加 `preferred_opener` 的同时，支持对现有本地 workspace 文件的向后兼容解析
- [x] 1.3 验证支持的打开程序值：`codex`、`claude`、`github-copilot` 和 `editor`
- [x] 1.4 将 `editor` 映射到 `kind: editor, id: vscode`
- [x] 1.5 将 agent 打开程序值映射到 `kind: agent` 及匹配的 `id`
...
- [x] 3.2 创建或刷新根目录的 `AGENTS.md`，包含 OpenSpec 管理 workspace 指导块
- [x] 3.3 保留用户编写的 `AGENTS.md` 中受管理块之外的内容
- [x] 3.4 将受管理块附加到未标记的现有 `AGENTS.md` 文件
- [x] 3.5 在 workspace 根目录创建或刷新 `<workspace-name>.code-workspace`
- [x] 3.6 在 `.code-workspace` 中包含 workspace 根目录和每个具有有效本地路径的链接 repository 或文件夹
- [x] 3.7 从 `.code-workspace` 中省略本地路径缺失或无效的链接 repository 或文件夹
...
- [x] 4.4 从 workspace 文件夹或子目录运行时，打开当前 workspace
- [x] 4.5 在 workspace 外部运行时，自动选择唯一已知的 workspace
- [x] 4.6 当有多个已知 workspace 时，显示交互式选择器
- [x] 4.7 在非交互式 schema 下报告模糊的 workspace 选择，并列出已知 workspace 名称
- [x] 4.8 清晰报告未解析的 workspace 选择，并建议 `openspec workspace setup`
- [x] 4.9 处理不支持的 `--prepare-only`、`--json` 和 `--change` 标志并给出清晰错误
- [x] 4.10 为选择、冲突、不支持标志和无 workspace 情况添加命令集成测试

## 5. 打开程序解析

- [x] 5.1 在 workspace 本地偏好之前解析命令行打开程序覆盖
...
- [x] 5.8 在非交互式 schema 下报告未设置的打开程序偏好，并给出覆盖指导
...
- [x] 6.3 从 workspace 根目录启动 Codex，并附加有效的链接路径
- [x] 6.4 从 workspace 根目录启动 Claude，并附加有效的链接路径
- [x] 6.5 当 agent CLI 需要初始提示参数时，使用最简启动提示
...
- [x] 7.1 更新 workspace 命令帮助，涵盖 setup `--opener`、open 位置参数、`--workspace`、`--agent` 和 `--editor`
- [x] 7.2 为新的 workspace 打开表面更新命令注册表和 shell 补全元数据
- [x] 7.3 更新 workspace 文档，描述首选打开程序、编辑器打开、agent 打开和 `.code-workspace` 行为
- [x] 7.4 记录 `.code-workspace` 是机器本地的，默认被忽略
- [x] 7.5 记录根 workspace 打开支持探索和 planning，实现从显式用户请求开始

## 8. 验证

- [x] 8.1 运行 `node bin/openspec.js validate workspace-open-agent-context --strict`
- [x] 8.2 运行针对性 workspace 命令测试
- [x] 8.3 运行针对性 workspace 基础测试
- [x] 8.4 运行涵盖 Codex、Claude、GitHub Copilot 和 VS Code 编辑器路径的命令生成或启动器测试
- [x] 8.5 为 workspace 打开表面运行跨平台路径测试
