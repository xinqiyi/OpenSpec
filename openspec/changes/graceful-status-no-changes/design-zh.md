...
该错误在上手期间（问题 #714）出现，当 AI 代理在创建任何变更之前调用 `openspec status` 时。

## 目标 / 非目标

**目标：**
- 使 `openspec status` 在不存在变更时以退出码 0 和友好消息退出
- 支持无变更情况下的文本和 JSON 输出模式
- 保持所有其他命令的验证行为不变
...
- **[风险]** 未来其他命令也可能受益于优雅的无变更处理 -- 缓解措施：`getAvailableChanges` 现在是公开且可复用的，使得在其他地方应用相同模式变得容易。
