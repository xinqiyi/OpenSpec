...
- [x] 1.2 在文本 schema 下：打印 `没有活跃变更。使用以下命令创建一个：openspec new change <name>` 并返回（退出码 0）
- [x] 1.3 在 JSON schema 下：输出 `{"changes":[],"message":"没有活跃变更。"}` 并返回（退出码 0）

## 2. 测试

- [x] 2.1 添加测试：`openspec status` 无变更时以友好消息优雅退出（文本 schema）
