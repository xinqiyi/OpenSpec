...
我们希望将 `spec-driven` 重命名为 `openspec-default`，以更好地反映它是标准/默认 workflow。然而，直接重命名会破坏现有项目中在 `openspec/config.yaml` 中设置了 `schema: spec-driven` 的项目。添加别名支持允许两个名称互换使用，从而实现平滑过渡，不会产生破坏性变更。

## 变更内容

- 在 schema 解析器中添加 schema 别名解析功能
- `openspec-default` 和 `spec-driven` 都将解析为同一 schema
...
- `schema-aliases`：支持 schema 名称别名，使多个名称可以解析到同一 schema 目录
