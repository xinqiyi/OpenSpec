...
OpenSpec 目前为每个用户安装 10 个工作流（技能 + 命令），让新用户不知所措。Init 流程会问多个问题（配置文件、交付方式、工具），在用户体验到价值之前就造成了摩擦。
...
- `src/core/templates/workflows/*.ts` - 各个工作流模板
...
- 创建新的 `propose` 工作流，结合 `new` + `ff`
- 修复工具选择 UX（空格选择，回车确认）
- 保持对现有用户的向后兼容性

**非目标：**
- 移除任何现有的工作流（所有工作流仍可通过自定义配置文件使用）
...
**理由：** 全局配置已经存在，具有 XDG/APPDATA 跨平台路径处理、模式演进和合并默认值行为。复用它可以避免第二个配置文件并利用现有基础设施。
...
**考虑的替代方案：**
- 三层（core/extended/custom）：Extended 是冗余的——想要所有工作流的用户可以在 custom 中选择它们
- 单独的 profile 和 delivery 命令：合并到一个选择器中减少认知负担

### 3. Propose 工作流 = New + FF 合并

一个工作流，一键创建变更并生成所有工件。

**理由：** 大多数用户希望从想法直接到可实现状态。将 `new`（创建文件夹）和 `ff`（生成工件）分开增加了不必要的步骤。想要控制的 power 用户可以通过自定义配置文件使用 `new` + `continue`。
...
- 额外的工作流（不在配置文件中）被保留
- 交付更改被应用：切换到 `skills` 移除命令，切换到 `commands` 移除技能
...
文件系统方法避免了所有三个问题。对于团队来说，实际上是有益的：检入的技能文件意味着任何团队成员运行 `openspec update` 都会刷新项目支持的所有工具的技能。生成的文件既作为交付物，也作为隐式的工具清单。

已知缺陷：将配置存储在项目树之外的工具（没有要扫描的本地目录）需要特定工具的处理，因为项目中没有什么可扫描的。如果支持此类工具，届时再处理。

**何时使用 init vs update：**
- `init`：首次设置，或想要更改配置了哪些工具时
- `update`：更改配置后，或想将模板刷新到最新版本时

### 8. 现有用户迁移

当 `openspec init` 或 `openspec update` 遇到具有现有工作流但全局配置中没有 `profile` 字段的项目时，它执行一次性迁移以保留用户的当前设置。

**理由：** 没有迁移，现有用户将默认为 `core` 配置文件，导致 `propose` 被添加到他们现有的 10 个工作流之上——使情况更糟，而不是更好。迁移确保现有用户精确保留他们拥有的内容。
...
**扫描逻辑：**
- 扫描所有工具目录（`.claude/skills/`、`.cursor/skills/` 等）以查找工作流目录/文件
- 仅匹配 `ALL_WORKFLOWS` 常量——忽略用户创建的自定义技能/命令
- 将目录名称映射回工作流 ID（例如 `openspec-explore/` → `explore`、`opsx-explore.md` → `explore`）
- 取跨所有工具的检测到的工作流名称的并集

**边界情况：**
- **用户手动删除了一些工作流：** 迁移扫描实际安装的内容，尊重用户的选择
- **具有不同工作流集的多个项目：** 第一个触发迁移的项目设置全局配置；后续项目使用它
- **用户在目录中有自定义（非 OpenSpec）技能：** 被忽略——扫描器仅匹配 `ALL_WORKFLOWS` 中已知的工作流 ID
...
- 不迁移，仅仅默认为 core：破坏现有用户，添加 `propose` 并显示"额外工作流"警告
- 在全局配置读取时迁移：过于隐式，难以向用户显示反馈

### 9. 模板中的通用下一步指导

工作流模板使用通用的、基于概念的下一步指导，而不是引用特定的工作流命令。例如，模板说"创建变更提案"而不是"运行 `/opsx:propose`"。

**理由：** 条件性交叉引用（每个模板检查安装了哪些其他工作流并渲染不同的命令名称）给模板生成、测试和维护增加了显著复杂度。通用指导完全避免了这个问题，同时仍然有用——用户已经知道他们安装了哪些工作流。
...
**理由：** 配置文件和 delivery 更改可能需要文件添加/移除操作，即使现有的技能模板是最新的。如果我们只检查模板版本，update 可能错误地返回"已是最新"并跳过所需的同步。

**实现：**
- 保留版本检查用于模板刷新决策
- 为配置文件/delivery 添加文件状态漂移检查（缺失预期文件或来自已移除 delivery 模式的过期文件）
- 将版本漂移或配置漂移视为需要更新

### 11. 工具配置检测包括仅命令安装

用于 update 的已配置工具检测必须包括命令文件，而不仅仅是技能文件。

**理由：** 使用 `delivery: commands`，一个项目可以在没有技能文件的情况下完全配置。仅技能检测会错误地报告"未找到已配置的工具。"

**实现：**
- 对于 update 流程，如果某个工具有生成的技能或生成的命令，则视其为已配置
- 保持迁移工作流扫描行为不变（技能仍然是迁移的事实源）
...
**风险：破坏现有用户的工作流程**
→ 缓解措施：文件系统是事实源，现有安装保持不变。所有工作流可通过自定义配置文件使用。

**风险：Propose 工作流重复 ff 逻辑**
→ 缓解措施：将共享的工件生成提取为可复用的函数，`propose` 和 `ff` 都调用它。
...
→ 可接受：这些覆盖了主要循环。需要更多工作流的用户可以使用 `openspec config profile` 选择额外的工作流。
...
**回滚：** 所有更改都是增量式的。现有行为通过选择所有工作流的自定义配置文件得以保留。
