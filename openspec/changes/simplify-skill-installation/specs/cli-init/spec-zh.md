...
### 需求：按工具生成 skill（替换固定的 9 skill 要求）
init 命令应根据活跃配置文件生成 skill，而不是固定的集合。

#### 场景：核心配置文件 skill 生成
- **WHEN** 用户使用配置文件 `core` 运行 init
- **THEN** 系统应为 CORE_WORKFLOWS 常量中的 workflow 生成 skill：propose、explore、apply、archive
- **THEN** 系统不应为配置文件之外的 workflow 生成 skill

#### 场景：自定义配置文件 skill 生成
- **WHEN** 用户使用配置文件 `custom` 运行 init
- **THEN** 系统仅应为配置 `workflows` 数组中列出的 workflow 生成 skill

#### 场景：在 skill template 中包含 Propose workflow
- **WHEN** 生成 skill
- **THEN** 系统应将 `propose` workflow 作为可用 skill template 包含在内
...
#### 场景：仅命令交付
- **WHEN** 交付设置为 `commands`
- **THEN** 系统不应生成任何 skill 文件

#### 场景：两者交付
- **WHEN** 交付设置为 `both`
- **THEN** 系统应为配置文件 workflow 同时生成 skill 和命令文件

#### 场景：在命令 template 中包含 Propose workflow
- **WHEN** 生成命令
- **THEN** 系统应将 `propose` workflow 作为可用命令 template 包含在内
...
- **WHEN** 用户在具有现有 workflow 文件的项目上运行 `openspec init`
- **AND** 全局配置不包含 `profile` 字段
- **THEN** 系统应在继续之前执行一次性迁移（参见 `specs/cli-update/spec.md`）
- **THEN** 系统应使用迁移后的配置继续 init

#### 场景：在新项目上初始化（无现有 workflow）
- **WHEN** 用户在没有现有 workflow 文件的项目上运行 `openspec init`
- **AND** 全局配置不包含 `profile` 字段
- **THEN** 系统不应执行迁移
...
#### 场景：用户有配置文件偏好
- **WHEN** 全局配置包含带有自定义 workflow 的 `profile: "custom"`
- **THEN** init 应安装自定义配置文件 workflow

#### 场景：用户有交付偏好
- **WHEN** 全局配置包含 `delivery: "skills"`
- **THEN** init 应仅安装 skill 文件，而非命令
...
- **AND** 全局配置指定了带有 workflow 的 `profile: "custom"`
- **THEN** 系统应直接使用自定义配置文件 workflow
- **AND** 系统不应显示配置文件确认提示
...
### 需求：Init 保留现有 workflow
init 命令不应移除已安装的 workflow，但应遵循交付设置。

#### 场景：现有自定义安装
- **WHEN** 用户具有带有额外 workflow 的自定义配置文件，并使用核心配置文件运行 `openspec init`
- **THEN** 系统不应移除额外的 workflow
- **THEN** 系统应重新生成核心 workflow 文件，用最新 template 覆盖现有内容
...
- **THEN** 这适用于所有 workflow，包括配置文件中没有的额外 workflow

#### 场景：即使 template 是最新的，重新初始化也应用交付清理
- **WHEN** 用户在现有项目上运行 `openspec init`
- **AND** 现有文件已经是最新 template 版本
- **AND** 自上次初始化以来交付已更改
