...
- [x] 1.1 在 `src/core/global-config.ts` schema 中扩展 `profile`、`delivery` 和 `workflows` 字段
...
- [x] 3.9 处理非交互 schema：报错并显示有帮助的消息
...
- [x] 5.2 实现结合 new + ff 行为的 skill template
- [x] 5.3 确保 propose 在生成 artifact 之前通过 `openspec new change` 创建 `.openspec.yaml`
- [x] 5.4 为 template 添加入门式解释性输出
- [x] 5.5 实现 propose 的命令 template
- [x] 5.6 从 `src/core/templates/skill-templates.ts` 导出 template
- [x] 5.7 在 `src/core/shared/tool-detection.ts` 的 `SKILL_NAMES` 中添加 `openspec-propose`
- [x] 5.8 在 `src/core/shared/skill-generation.ts` 的命令 template 中添加 `propose`
- [x] 5.9 在 `src/core/shared/tool-detection.ts` 的 `COMMAND_IDS` 中添加 `propose`
- [x] 5.10 为 propose template 添加测试（创建变更、生成 artifact、等同于 new + ff）
...
- [x] 6.4 添加当 delivery 为 'commands' 时跳过 skill 生成的逻辑
...
- [x] 7.8 更新非交互 schema 以使用默认值，不进行提示
...
- [x] 8.4 显示迁移消息："已迁移：包含 N 个 workflow 的自定义配置文件" + "此版本新增：/opsx:propose。尝试运行 'openspec config profile core' 以获得精简体验。"
- [x] 8.5 添加项目检查：如果不存在 `openspec/` 目录则退出并报错
- [x] 8.6 添加检测逻辑，找出配置中存在但未安装的 workflow（以便添加）
- [x] 8.7 添加检测逻辑，找出已安装且需要刷新的 workflow（以便更新）
- [x] 8.8 尊重 delivery 设置：如果为 `skills` 则仅生成 skill，如果为 `commands` 则仅生成命令
- [x] 8.9 当 delivery 更改时删除文件：如果为 `skills` 则移除命令，如果为 `commands` 则移除 skill
- [x] 8.10 为配置文件中缺失的 workflow 生成新的 workflow 文件
- [x] 8.11 显示摘要："已添加：X、Y" / "已更新：Z" / "已移除：N 个文件" / "已是最新。"
- [x] 8.12 在输出中列出受影响的工具："工具：Claude Code、Cursor"
- [x] 8.13 检测当前未配置的新工具目录，并显示重新初始化提示
- [x] 8.14 为迁移场景添加测试（现有用户、部分 workflow、多工具、幂等性、忽略自定义 skill）
...
<!-- - [ ] 10.2 更新 ff skill 以验证 `openspec new change` 后 `.openspec.yaml` 是否存在 -->
<!-- - [ ] 10.3 为 skill 添加防护："切勿手动在 openspec/changes/ 中创建文件——请使用 openspec new change" -->

## 11. template 下一步指导

- [x] 11.1 审计所有 template，检查硬编码的跨 workflow 命令引用（例如 `/opsx:propose`）
- [x] 11.2 将所有特定命令引用替换为基于概念的通用指导（例如"创建变更 proposal"）
...
- [x] 13.1 确保即使 template 是最新的，`update` 也将 profile/delivery 漂移视为需要更新
- [x] 13.2 确保 `update` 将仅命令安装视为已配置的工具
- [x] 13.3 确保 `init` 验证 `--profile` 值并在无效覆盖时报错
- [x] 13.4 确保重新运行 `init` 时应用 delivery 清理（移除与当前 delivery schema 不匹配的文件）
