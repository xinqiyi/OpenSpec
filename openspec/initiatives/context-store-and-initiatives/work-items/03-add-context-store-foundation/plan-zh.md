...
为上下文存储添加最小的核心基础，而不让存储层知道倡议、集合、workspace 或 repository 本地变更。
...
- 在此切片中不添加倡议、集合、workspace 打开、同步、拉取、推送或 CLI 行为。
...
- 未添加 Git 克隆、拉取、推送、同步、workspace 状态、集合清单或 CLI 行为。
