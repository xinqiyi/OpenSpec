...
创建/列出操作和 CLI 适配器切片已完成。完整的读取/显示、更新和删除策略推迟到后续的代理优先发现和生命周期工作。
...
为协调工作提供一个持久、共享、代理可消费的家园，位于 `initiatives/` 集合内。
...
- 仓库本地 OpenSpec 变更仍然是实施构件；倡议协调意图、决策、问题和任务。
- 在此事项中不实现工作区打开、仓库解析、状态仪表板、同步或链接变更生命周期。
...
- 使用严格的 YAML 解析和验证，遵循现有的基础架构模式。
- 不在倡议 MVP 中创建 `links.yaml`。仓库变更连接属于后续的工作区/本地协调工作。
- 保持 Markdown 验证轻量；生成有用的结构，但暂不验证散文内容。
- 在挂载集合操作之前，从倡议架构和模板辅助函数开始实施。
...
- 完整的倡议显示/读取行为属于代理优先的倡议发现，等待返回形状更加清晰。
...
- `tasks.md`：仅协调任务，非仓库实施任务。

推迟 `contracts/`、`README.md`、里程碑、依赖图、外部问题链接、工作区路径映射、状态仪表板、`links.yaml` 和 Markdown 内容验证。
...
- 添加类型、常量、ID 验证、严格的 `initiative.yaml` 解析/序列化辅助函数和默认模板构建器。
- 在架构和模板锁定后，添加创建/列出挂载集合操作。
...
在第一个 CLI 切片中保持存储选择显式。要求使用 `--store <id>` 或 `--store-path <path>`，拒绝同时使用两者，暂不添加当前目录发现、单存储自动选择、交互式选择器、全局默认存储或工作区选定存储状态。
...
- 返回工作区风格的 JSON 负载，带有顶层的 `status` 诊断数组。
