...
该项目存在的原因是，代理优先的倡议工作流在仓库本地交接和工作区打开变得连贯之前，需要一个可用的共享存储。

## 目标

让用户或代理能够创建、注册、列出和诊断本地上下文存储，而无需了解内部注册表布局。
...
该工作项应提供这些原语。它不应实现仓库本地倡议关联、倡议解析、工作区打开或进度/状态仪表板。

## 当前锁定的方向

- 目前保留面向用户的术语 `store`；命名完善被推迟。
- 在此切片中使用 `context-store` 作为顶层 CLI 命名空间。它对代理更明确，避免了重载宽泛的顶层 `store` 命令。仅在上下文已限定时保留 `store` 作为简写，例如 `initiative list --store <id>`。
- `context-store setup <id>` 应创建或使用本地文件夹，写入可移植的存储元数据，注册本地路径，并可选择初始化 Git。
- 当 `--path` 省略时，`context-store setup <id>` 应默认使用 `./<id>`。
- 使用当前目录应是显式的 `--path .`；设置不应将当前仓库静默转换为上下文存储。
...
- 当元数据缺失时，默认从仓库或文件夹名称推断存储 ID。
- 可移植的存储元数据确切为 `.openspec-store/store.yaml`。它应被检入上下文存储仓库，目前仅包含可移植标识：
...
- 存储在机器本地注册表中注册。
- 交互式 TTY 模式在既未提供 `--init-git` 也未提供 `--no-init-git` 时提示 Git 初始化；默认答案为是。
- `--json`、非 TTY 执行、`--init-git` 和 `--no-init-git` 不会提示。
- 仅在提示答案为是或传递了 `--init-git` 时才初始化 Git。
- 设置不提交、推送、拉取、创建远程或创建托管仓库。
...
- 如果元数据缺失，除非传递了 `--id`，否则从文件夹或仓库名称推断 ID。
- 推断直接使用文件夹或仓库名称原样，然后应用正常的上下文存储 ID 验证。在此切片中不要做巧妙的规范化。
- 缺失的元数据会被写入。
- 机器本地注册表会被更新。
- 相同 ID 和相同路径是幂等的成功。
...
`initiative list` 成为跨已注册存储的代理友好发现命令。
...
- 过滤后的 `--store` 或 `--store-path` 模式在该存储无法读取时直接失败，因为没有后备存储。
...
- 倡议本地解析
- 工作区倡议打开
- 同步、拉取、推送、远程仓库创建或冲突处理

## 剩余决策

实施前无待定项。JSON 格式可以遵循现有命令模式：顶层结果对象加上 `status` 诊断数组。部分成功返回退出码 0 并附带警告诊断；完全失败返回非零退出码。
