...
- `initiative show <id>` 应作为 agent 的定位/发现命令。
- 该命令应回答用户指的是哪个倡议、权威上下文在哪里、以及倡议元数据在哪里。
- 该命令不应拼接 markdown、总结倡议内容、计算工作进度、解析本地 repository、列出关联变更或打开 workspace。
- 默认查找应搜索所有已注册的上下文存储。
...
- `initiative.root` 足以让 agent 使用常规文件系统工具检查文件夹。
...
- 成功时的人类可读输出应是紧凑的定位器视图：标题、ID、摘要、上下文存储、位置和 spec 文件名。
...
- 其他倡议文件是 schema/配置相关的，不应硬编码到 `show` 中。
