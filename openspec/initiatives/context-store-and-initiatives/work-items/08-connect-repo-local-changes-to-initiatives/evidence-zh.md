...
倡议链接应位于仓库本地变更的 `.openspec.yaml` 中。
...
这既保持了仓库的实现所有权，又保留了对规范化倡议上下文的持久引用。
...
- 归档操作会移动整个变更目录，因此倡议链接将随已归档的变更一起移动。
...
这保持操作归仓库所有。倡议是变更上的一个引用，而非创建或拥有变更的行为者。

第一个切片还应为 `new change` 添加 `--json`，以便代理能够捕获创建的变更路径、元数据路径和倡议引用。
...
在倡议查找完成且无歧义之前，不应写入仓库本地元数据。

## 决策 4：V1 仅限仓库本地

Item 8 应仅支持仓库本地变更的倡议链接。

如果 `openspec new change <id> --initiative ...` 从工作区规划家园运行，v1 应拒绝并告知用户从拥有实现计划的仓库运行该命令。

现有的工作区规划变更保持兼容性行为，在此切片中不应获得倡议链接功能。

这保持了边界：倡议协调共享上下文，仓库本地变更拥有实现计划，工作区打开本地视图。

## 决策 5：V1 中无仓库拥有权匹配

Item 8 不应验证当前仓库是否被倡议命名、拥有或从中推断。

创建带有倡议链接的仓库本地变更记录的是参与该倡议的行为。它不证明拥有权、仓库影响或倡议区域的覆盖范围。

仓库拥有权匹配可以在倡议解析或显式倡议元数据拥有真实的仓库/区域模型后再行考虑。
...
输出不应包含 `next` 或其他建议的工作流操作。API 响应应报告操作结果或错误；选择下一步操作是代理的责任，取决于更广泛的上下文。

## 决策 7：现有变更恢复

Item 8 应包含一个友好的恢复命令，用于现有仓库本地变更：
...
此命令是一个经过验证的设置器，用于已检入的仓库本地变更元数据。在 Item 8 中，唯一支持的设置字段是倡议链接，唯一可以修改的文件是 `openspec/changes/<id>/.openspec.yaml`。
...
状态和指令应显示仓库本地变更已链接到某项倡议，但不应显示或解析倡议本身。

人类可读的状态输出应显示存储的倡议引用，JSON 状态输出应包含存储的倡议 `{ store, id }`。指令输出应包含一条简洁的事实性说明，告知该变更已链接到倡议。

在 v1 中，状态和指令不应从上下文存储中读取、总结、验证或解析倡议。缺失或不可用的上下文存储不应导致仓库本地状态或指令失败。

这使该关系在普通仓库本地工作流中保持可见，同时保留边界：倡议查找和上下文读取属于倡议专用命令。
...
- 轻量发布应测试倡议关联的仓库本地变更是否有用，然后再添加门控、拥有权推断或更广泛的工作流集成。
- 独立的 `initiative resolve` 后来被否决；工作区本地视图状态拥有本地路径映射。
...
- `openspec new change <id> --initiative ...` 用于仓库本地变更，支持 `--json`、`--store` 和 `--store-path`。
- `openspec set change <id> --initiative ...` 用于现有仓库本地变更。
- 在 `initiative: { store, id }` 下的可检入可移植元数据。
- 仅从存储元数据中获取状态和指令可见性。
- 工作区拒绝、查找失败不写入、相同链接幂等性以及不同链接冲突保护。
