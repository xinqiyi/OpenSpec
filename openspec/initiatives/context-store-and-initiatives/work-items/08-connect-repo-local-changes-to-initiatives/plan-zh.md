...
```text
倡议协调共享上下文。
repository 本地变更拥有实现计划。
workspace 打开本地视图。
```

## 目标

让 agent 能够创建或链接 repository 本地的 OpenSpec 变更到共享倡议，而无需复制倡议正文、存储机器本地路径或使倡议拥有 repository 实现产物。

示例用户提示：

```text
使用倡议 billing-launch，为 API 工作创建一个 proposal。
```
...
决策：将倡议链接存储在 repository 本地变更的 `.openspec.yaml` 中。
...
- 该链接是持久的 repository 上下文，应与变更一起检入。
- spec 化的倡议上下文仍保留在上下文存储中。
- 元数据在团队成员和机器之间保持可移植。

### 2. 创建命令形态

决策：通过 `--initiative` 将倡议链接添加到 repository 本地变更创建命令。
...
- 命令从 `new change` 出发，因为变更归 repository 所有。
- `--initiative` 修改 repository 本地变更创建；它不会让倡议创建或拥有该变更。
- `--json` 应添加到 `new change`，用于 agent 可读的交接输出。
- 单独的倡议拥有的创建命令不在第一个切片的范围内。

理由：

- 预期的用户流程是 agent 优先："使用倡议 X，为 repository 工作创建一个 proposal。"
- agent 需要一个普通的 repository 本地创建命令，同时也能写入倡议引用。
- 将动词根植于 `new change` 保持了变更实现 repository 所有切片的边界。
...
- 在查找完成且无歧义之前，不写入 repository 本地倡议元数据。

理由：

- agent 可以在安全的情况下使用简短形式。
- 持久的 repository 本地链接不应基于部分知识创建。
- 该行为与现有的 agent 优先发现语义一致。

### 4. V1 仅限 repository 本地

决策：Item 8 仅支持 repository 本地变更的倡议链接。

规则：

- `openspec new change <id> --initiative ...` 仅当当前 planning 家园是 repository 本地时才创建倡议关联的变更。
- 如果命令从 workspace planning 家园运行，v1 拒绝并提供清晰指导，告知用户从拥有实现计划的 repository 运行该命令。
- 现有的 workspace planning 变更保持兼容性行为，在此切片中不扩展倡议链接功能。

理由：

- 当前产品边界分配实现计划给 repository 本地的 OpenSpec 变更。
- workspace 是本地视图，而非倡议工作的持久 planning 所有者。
- 扩展 workspace planning 变更将复活已被取代的 workspace 拥有计划模型。

### 5. repository 拥有权匹配

决策：v1 中不尝试 repository 拥有权匹配。

规则：

- 创建带有倡议链接的 repository 本地变更记录的是对倡议的参与。
- 该链接不声明 OpenSpec 已验证了 repository 拥有权、repository 影响或倡议区域覆盖范围。
- 命令不应仅因为当前 repository 不在倡议内容中而阻塞或警告。

理由：

- Item 8 不应发明 repository 拥有权或单体 repository 区域语义。
- 拥有权匹配属于后续的倡议解析或显式倡议元数据。
- 保持 v1 小巧让团队在添加策略门控之前测试关联的 repository 本地变更是否有用。
...
- 输出应报告命令执行的操作，而非推荐 workflow 下一步。
- 人类可读输出应确认创建的变更位置、schema 和倡议链接。
- JSON 输出应包含已创建变更和倡议链接的稳定字段。
- JSON 输出不应包含 `next` 命令或建议的 workflow 操作。
- 输出不应包含倡议摘要、repository 拥有权声明、已解析的本地上下文存储路径或进度/状态类字段。
...
- 准确选择下一步操作取决于 agent 上下文，应保持为 agent 的责任。
- 保持输出事实性可避免将变更创建与后续生命周期设计耦合。

### 7. 现有变更恢复

决策：包含一个恢复命令，用于在现有 repository 本地变更上设置倡议链接。
...
- `openspec set change <id> --initiative ...` 是一个经过验证的 repository 本地变更元数据设置器。
...
- 如果命令从 workspace planning 家园运行，它拒绝的原因与倡议关联的 `new change` 相同。

理由：

- agent 可能在变更创建时忘记传递 `--initiative`；v1 需要一个友好的恢复路径。
...
- 缺失或不可用的上下文存储不得导致 repository 本地状态或指令失败。
- 输出不应添加下一步建议。

理由：

- 倡议链接应在普通 repository 本地 workflow 输出中可见，以便用户和 agent 不会错过该关系。
- 将可见性限制为存储的元数据可避免将上下文存储可用性引入为 repository 本地 workflow 命令的依赖。
