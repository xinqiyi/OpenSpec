...
- 意外的设置之前在当前 repository 下创建了一个存储，然后管理的默认值才被纠正；
- 清理没有 CLI 路径，需要手动删除文件并编辑注册表；
- Git 初始化使共享文件处于未跟踪状态，没有告诉用户或 agent 下一步该做什么。
...
- 仅提示本地设置选项；
- 从不隐式推送、拉取、提交、创建远程 repository 或删除文件；
- 保持 JSON 输出足够明确，以便 agent 安全继续；
...
- 另一个 Git repository 内部的显式设置路径在非交互 schema 下被阻止，在交互 schema 下需要显式确认。
