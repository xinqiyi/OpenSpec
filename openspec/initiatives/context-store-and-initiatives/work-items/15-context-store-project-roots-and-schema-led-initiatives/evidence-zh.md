...
- 一个全新代理风格的提示成功在已注册的 `team-context` 上下文存储中创建了 `agent-trace-hooks`。
- 当前的 CLI 创建了以下硬编码文件集：
...
- 生成的 Markdown 模板以 `TBD` 占位符开头。
- 代理填充了这些文档，内容看似合理但未经审阅的规划内容。
- 我们手动将测试倡议缩减为精简形态：
...
- 倡议 Markdown 模板在 `src/core/collections/initiatives/templates.ts` 中硬编码。
- `createInitiative` 在 `src/core/collections/initiatives/operations.ts` 中写入 `initiative.yaml`，然后写入所有默认模板文件。
...
- 变更产物状态/指令通过 `src/core/artifact-graph/instruction-loader.ts` 与仓库本地变更上下文耦合。
- 规划家园检测当前将包含 `openspec/` 的祖先视为可能的仓库规划根，因此向上下文存储添加配置需要安全检查。
...
文档应避免说倡议仅用于跨仓库或跨团队工作。用户选择上下文存储可能仅仅是因为他们希望将 OpenSpec 产物放在实现仓库之外。

`initiative create` 应创建最小可用的共享对象，然后通过状态/指令教会代理如何继续。它不应在审阅之前假装需求、决策和任务已经存在。
...
- 当前断言六文件 MVP 契约的测试
- 当前告诉代理编辑五个生成的 Markdown 文件的文档和生成的代理指导
- 倡议规划产物与仓库本地实现任务之间的歧义
- 上下文存储根在获得 `openspec/config.yaml` 后意外成为仓库规划家园
...
- 添加倡议特定的产物上下文，而不是强制倡议通过仓库本地变更上下文。
- 保护规划家园发现，使拥有 `openspec/config.yaml` 的上下文存储不会成为意外的实现仓库。

UX/产品检查：

- 将上下文存储描述为 OpenSpec 管理的规划家园。它可以用于跨仓库协调，但也可以仅仅是为了将 OpenSpec 产物保留在实现仓库之外。
- 使 `initiative create` 精简：`initiative.yaml` 加上一个种子产物如 `brief.md`。
- 添加状态/指令输出，使代理仅在存在已审阅内容需要捕获时才创建需求和设计产物。
- 停止将默认的倡议产物视为用户或代理应立即填写的文件。
