...
手动 beta 测试证据：代理可以创建当前的 MVP 倡议形态，但生成的 `requirements.md`、`design.md`、`decisions.md`、`questions.md` 和 `tasks.md` 会引发过早的、未经审阅的规划内容。

## 为何存在

`openspec initiative create` 当前从硬编码的 TypeScript 常量和 `TBD` 模板创建看起来完整的规划包。这使 MVP 具象化，但对于真实的倡议工作来说，它是错误的默认行为。

倡议是面向 PM、设计师、架构师和代理的高级共享规划界面。它们应捕获意图、已审阅的需求、设计方向、待解决的问题和决策，随着这些产物变得真实而记录。它们不应仅仅因为文件夹存在就创建看起来已完成但实际上空白的或虚假的文档。

更广泛的产品形态是，上下文存储应像一个在 `openspec init` 之后的仓库一样，感觉像一个 OpenSpec 根：它可以拥有本地 OpenSpec 配置和项目本地 schema。区别在于生命周期：上下文存储是共享的上下文根，而非实现仓库。
...
上下文存储内的 `openspec/` 目录用于 OpenSpec 配置和 schema 解析。它本身不会使上下文存储成为可执行的实现规划家园。
...
- 不要添加跨仓库的 apply、archive、validation 或 spec-sync 编排。
- 不要使工作区本地产物成为共享规划的真实依据。
- 不要自动迁移现有倡议。
...
version: 1
description: 面向 PM、设计师、架构师和代理的高级倡议规划
usage: initiative
artifacts:
  - id: requirements
...
不要在默认倡议 schema 中包含 `tasks.md`。实现任务属于仓库本地变更。协调任务、工作流、决策日志和问题日志可以是单独的后备 schema 或显式产物，等待使用模式更加清晰后再行添加。
...
这避免重载现有仓库本地的 `schema` 字段（当前含义为"默认变更 schema"）。如果实现选择复用 `schema`，文档和 JSON 输出必须显式说明上下文存储的作用域。
...
代理仅在对话中有足够已审阅内容时才编写 `requirements.md`。`design.md` 在需求存在后变得就绪。
...
- 镜像仓库本地产物工作流但返回倡议特定字段的倡议 `status` 和 `instructions` 命令
- 一个仅写入 `initiative.yaml` 和 `brief.md` 的精简 `initiative create` 路径

不要不修改地使用现有的仓库规划家园解析器。一旦上下文存储包含 `openspec/config.yaml`，当前的"最近的 `openspec/` 文件夹意味着仓库规划家园"启发式方法可能意外使上下文存储看起来像实现仓库。此项工作必须：

- 教导规划家园解析检测 `.openspec-store/store.yaml`，并为实现命令返回或拒绝"上下文存储"类型，或
...
- 继续使用现有的 `schema` 作为默认的仓库本地变更 schema。
...
最简单的面向用户模型仍然是 `openspec/schemas/`，但命令必须避免将 `product-initiative` 列为有效的仓库本地变更工作流。
...
- 上下文存储获得 `openspec/config.yaml` 后的规划家园检测。
- 断言六文件 MVP 倡议形态的测试和文档。
- 如果倡议 schema 与变更 schema 共享相同命名空间，schema 列表和自动补全。
- 仍然告诉代理在创建后编辑每个生成的倡议 Markdown 文件的代理指导。
...
- 具有 `openspec/config.yaml` 的上下文存储根的规划家园测试
...
- 代理可以使用倡议状态/指令迭代地创建高级规划产物。
- `openspec new change` 不会仅因为存储有 `openspec/config.yaml` 而将上下文存储误认为是普通的实现仓库。
- 现有的 MVP 倡议继续可以列出和显示。
