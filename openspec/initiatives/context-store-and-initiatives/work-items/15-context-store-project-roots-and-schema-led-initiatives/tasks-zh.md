...
- [ ] 记录产品决策：上下文存储应像 OpenSpec 根一样表现用于配置和 schema 解析，但默认不作为实现 repository。
...
- [ ] 添加倡议产物指令输出，返回 schema 指导、template 内容、依赖关系、输出路径和现有路径。
- [ ] 确保可以为倡议产物指令读取存储本地配置上下文和规则，而不与 repository 本地变更配置混淆。
- [ ] 保护 planning 家园解析，使具有 `openspec/config.yaml` 的上下文存储不会静默成为 repository 本地的实现家园。
...
- [ ] 更新 beta 文档和 agent 指导，停止告诉 agent 在创建后立即编辑每个倡议 Markdown 文件。
