...
## 目的
定义 `/opsx:verify` 的行为，用于评估实现与变更工件之间的完成度、正确性和连贯性。

## 需求
### 需求：验证技能调用
系统应提供 `/opsx:verify` 技能，用于验证实现与变更工件的一致性。

#### 场景：提供变更名称时验证
- **WHEN** 代理执行 `/opsx:verify <change-name>`
- **THEN** 代理验证该特定变更的实现
- **AND** 生成验证报告

#### 场景：未提供变更名称时验证
- **WHEN** 代理执行 `/opsx:verify` 而不带变更名称
- **THEN** 代理提示用户从可用变更中选择
- **AND** 仅显示具有实现任务的变更

#### 场景：变更没有任务
- **WHEN** 选定的变更没有 tasks.md 或任务为空
- **THEN** 代理报告 "No tasks to verify"
...
#### 场景：规范覆盖检查
- **WHEN** 验证完成度
- **AND** 差异规范存在于 `openspec/changes/<name>/specs/` 中
- **THEN** 代理从差异规范中提取所有需求
- **AND** 在代码库中搜索每个需求的实现
...
- **WHEN** 发现问题
- **THEN** 按优先级分组并显示：
  1. 严重 - 归档前必须修复（缺失实现、未完成任务）
  2. 警告 - 应该修复（与规范/设计的差异、缺失测试）
  3. 建议 - 修复更好（模式不一致、小改进）
...
#### 场景：有规范但无设计的变更
- **WHEN** 变更有 tasks.md 和差异规范但没有 design.md
- **THEN** 验证完成度和正确性
- **AND** 跳过设计遵循检查
- **AND** 仍根据项目模式检查代码连贯性
